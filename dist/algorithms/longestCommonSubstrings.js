...
export function longestCommonElements(numPresent = 0, ...inputs) {
    const numInputs = inputs.length;
    if (typeof numPresent !== 'number')
        throw new TypeError('numPresent needs to be a number.');
    if (numInputs < 2)
        throw new RangeError('There needs to be at least 2 inputs.');
    if (numPresent > numInputs)
        throw new RangeError('numPresent needs to be less than or equal to the number of inputs.');
    if (numPresent < 0)
        throw new RangeError('numPresent needs to be greater than or equal to 0.');
    if (typeof inputs[0] === 'string') {
        return longestCommonSubstrings(numPresent || numInputs, ...inputs);
    }
    else if (Array.isArray(inputs[0])) {
        return longestCommonSubarrays(numPresent || numInputs, ...inputs);
    }
    else {
 ...
 * @returns {string[]}
 */
function longestCommonSubstrings(numPresent = 0, ...strings) {
    const numStrings = strings.length;
    const colorIndexes = [];
    const colorTracker = new Array(numStrings).fill(0);
    let colorIndex = 0;
    let concatString = '';
    ...
    for (let i = 0; i < numStrings; i++) {
        ...
        if (string.length === 0)
            throw new TypeError('Input strings must be nonempty.');
    ...
    if (numStrings >= maxSentinel.charCodeAt(0))
        throw new EvalError('Too many input strings, the number of available sentinel characters based on string content have been exceeded. Reduce the number of input strings or remove small char code characters from the input strings.');
    ...
    while (true) {
        ...
        if (colorCount < numPresent) {
            ...
            for (let i = windowStart + 1; i <= windowEnd; i++) {
                if (lcpArray[i][1] < minCommon)
                    minCommon = lcpArray[i][1];
            }
 ...
 * @returns {number[][]}
 */
function longestCommonSubarrays(numPresent = 0, ...arrays) {
    const numInputs = arrays.length;
    const colorIndexes = [];
    const colorTracker = new Array(numInputs).fill(0);
    let concatInput = [];
    let colorIndex = 0;
    for (let i = 0; i < numInputs; i++) {
        ...
        if (input.length === 0)
            throw new TypeError('Input array must be nonempty.');
        concatInput = concatInput.concat(...input.map(value => {
            if (typeof value !== 'number')
                throw new TypeError('Input must be an array of numbers.');
            if (value < 0)
                throw new RangeError('Input must be an array of positive numbers.');
...
function getColor(index, colorIndexes) {

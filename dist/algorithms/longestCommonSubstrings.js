 ...
 * @returns {string[]}
 */
export function longestCommonSubstrings(numPresent = 0, ...strings) {
    const numStrings = strings.length;
    if (typeof numPresent !== 'number')
        throw new TypeError('numPresent needs to be a number.');
    if (numStrings < 2)
        throw new RangeError('There needs to be at least 2 input strings.');
    if (numPresent > numStrings)
        throw new RangeError('numPresent needs to be less than or equal to the number of input strings.');
    if (numPresent < -1)
        throw new RangeError('numPresent needs to be greater than or equal to 0.');
    if (numPresent === 0)
        numPresent = numStrings;
    const colorIndexes = [];
    ...
    while (true) {
        if (windowEnd >= lcpArray.length - 1)
            break;

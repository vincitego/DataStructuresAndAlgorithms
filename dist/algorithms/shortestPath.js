...
export function shortestPath(graph, start, end) {
    ...
    if (!graphMap.has(start))
        throw new TypeError(`Start node id ${start} not found in graph.`);
    if (typeof end !== 'number')
        throw new TypeError(`End node id ${end} needs to be numeric.`);
    if (!graphMap.has(end))
        throw new TypeError(`End node id ${end} not found in graph.`);
    const previousNode = new Map(nodes.map(node => [node, -1]));

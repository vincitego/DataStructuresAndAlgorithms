...
export class FenwickTree {
    ...
    constructor(values) {
        ...
        for (let i = 1; i < treeLength; i++) {
            ...
            if (parentIndex < treeLength)
                this._tree[parentIndex] += this._tree[i];
        }
    }
    /**
    ...
    query(left, right) {
        if (typeof left !== 'number')
            throw new TypeError('Left needs to be a number');
        if (left < 1 || left > this.size())
            throw new RangeError('Left index out of range');
        if (right !== undefined) {
            if (typeof right !== 'number')
                throw new TypeError('Right needs to be a number');
            if (right < 1 || right > this.size())
                throw new RangeError('Right index out of range');
            if (left > right)
                throw new RangeError('Left index cannot be greater than right index');
        }
        return this._prefixSum(right !== null && right !== void 0 ? right : left) - this._prefixSum(left - 1);
    }
    /**
